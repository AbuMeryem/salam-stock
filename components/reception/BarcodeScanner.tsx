...
/**
 * Scanner code-barre v7.1 — caméra wild + zoom 2× + Vision Apple
 *
 * Bug v7 corrigé : iOS régénère les deviceId entre 2 appels getUserMedia,
 * donc warm-up + re-enumerate + open avec deviceId échoue avec
 * NotFoundError. On laisse iOS choisir la caméra (facingMode environment)
 * et on applique zoom 2× après coup pour compenser si iOS a pris
 * l'ultra-wide 0.5×.
 */
...
export function BarcodeScanner({
  ...
  async function startCamera() {
    ...
    stoppedRef.current = false;
    try {
      // PAS de deviceId enumerateDevices : iOS les régénère et ça plante
      // en NotFoundError. On laisse iOS choisir avec facingMode et on
      // compense via zoom advanced constraint après coup.
      ...
      // Capabilities → focus continu + zoom 2× + torch
      const caps = (
      ...
      // Décodage : BarcodeDetector natif > html5-qrcode fallback
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black flex flex-col">
      ...
      {/* SAISIE MANUELLE — ultime fallback */}
...
function humanError(raw: string): string {
  ...
  if (m.includes("notfound")) {
    return "Caméra introuvable. Vérifie que la caméra arrière fonctionne dans l'app Caméra iOS, puis recharge.";
  }
